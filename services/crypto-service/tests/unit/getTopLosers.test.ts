...
describe('getTopLosers (unit)', () => {
   ...
   */
  test('fetches markets, computes top losers, caches and returns result', async () => {
    const markets = [
      {
        id: 'btc',
        total_volume: 1000,
  ...
  test('returns unavailable result when upstream API fails', async () => {
    ...
    const result = await getTopLosers(axiosClient, 10);
